...
function SearchPets() {
	...
	async function formSubmit(data) {
		...
		const result = await searchPetsNearby(
			fullData.lat,
			fullData.lng,
			fullData.range
		);
			...
			<div className={css.root}>
				<div className={css["text-container"]}>
					...
					<Text variant="subtitle">
						{" "}
						Ingresa la siguente informacion para realizar la busqueda
					</Text>
				</div>
				<form
					id="my-form"
				...
				>
					<MapSearch lastCoords={handleCoordsChange}>
						Pon un punto de referencia en el mapa en donde desesas buscar
